...
══════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════
     FAQ — lazy init via IntersectionObserver
  ══════════════════════════════════════════════ */
  const initFAQ = () => {
  ...
  function updateCTA({ sym, name, patients, ticket, invLocal, breakEven, monthsToROI }) {
    ...
    if (els.ctaNumber) {
      els.ctaNumber.innerHTML =
        ...
        `el ${serviceName} se amortiza en <strong>${breakEven} consulta${breakEven !== 1 ? 's' : ''}</strong>.`;
    }
    if (els.ctaSubtitle) {
    ...
    const msg = `Hola, calculé en la web que con mi ticket promedio de ${fmt(ticket, sym)} ${name} el ${serviceName} ($${state.service} USD) se paga en ${breakEven} consultas (~${monthsToROI} mes${monthsToROI !== 1 ? 'es' : ''}). Me interesa saber más.`;
    if (els.waBtn) els.waBtn.href = 'https://wa.me/5491176199680?text=' + encodeURIComponent(msg);
  }
