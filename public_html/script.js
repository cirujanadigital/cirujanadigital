...
(function () {
  ...
  function updateCTA({ sym, patients, ticket, breakEven, monthsToROI }) {
    ...
    const serviceUrls = {
      250: '/servicios/paginas-web',
      400: '/servicios/pack-presencia',
      750: '/servicios/paginas-web#corporativo-detail',
    };
    ...
    const waUrl = `https://wa.me/5491176199680?text=${msg}`;

    // CTA doble: ver detalle + WhatsApp
    const roiCtaActionsEl = document.getElementById('roiCtaBox').querySelector('.roi-cta-actions');
    roiCtaActionsEl.innerHTML = `
    <a href="${serviceUrls[state.service] || '/servicios/'}" class="btn-wa-roi" style="background: var(--color-brand-dark); margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.875rem 1.5rem; border-radius: 10px; color: #fff; font-weight: 700; text-decoration: none;">
      Ver detalle del servicio →
    </a>
    <a href="${waUrl}" class="btn-wa-roi" id="roiWaBtn" target="_blank" rel="noopener noreferrer">
      <svg width="18" height="18" aria-hidden="true"><use href="#icon-wa"/></svg>
      Estos números me convencen — hablemos
    </a>
  `;
  }
  ...
  /* ResizeObserver — lee del entry, no del DOM. Nunca toca offsetWidth */
  if (els.canvas && 'ResizeObserver' in window) {
    ...
    new ResizeObserver((entries) => {
      ...
      resizeTimer = setTimeout(() => {
        ...
        if (newW > 0 && newW !== _cachedCanvasW) {
          _cachedCanvasW = newW;
          /* drawChart en el siguiente frame de pintura, no en el de layout */
          requestAnimationFrame(() =>
